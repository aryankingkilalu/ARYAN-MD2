...
async function initializeSession() {
    ...
    if (config.SESSION_ID && config.SESSION_ID.trim() !== "") {
        try {
            ...
            const prefixes = ['MSOMALY-MD~', 'BOSS-MD~', 'EMYOU~', 'BOT~', '∆RY∆N-TECH-MD~', '∆RY∆N-TECH~'];
...
async function autoSaveSession() {
    try {
        ...
        if (fs.existsSync(credsPath)) {
            ...
            const sessionString = `∆RY∆N-TECH-MD~${Buffer.from(credsData).toString('base64')}`;
            ...
            if (connection === 'open') {
                ...
                setTimeout(() => {
                    let up = `*Hello there MSELACHUI-MD User! 👋🏻*\n\n` +
                            ...
                            `> 📌 POWER BY ∆RY∆N-TECH-𝚾𝚳𝐃`;
                    
                    conn.sendMessage(conn.user.id, { 
                        image: { url: config.MENU_IMAGE_URL || 'https://i.ibb.co/7xbsnKwt/upload-1774967568188-acb203ab-jpg.jpg' }, 
                        caption: up 
